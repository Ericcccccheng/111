...
onBeforeUnmount(()=>{ 
  ...
  // -------------------------------------------------------------------------------------------
  function jumpToQueryPoint(){
  const qs = route.query
  const lat = parseFloat(String(qs.lat ?? ''))
  const lng = parseFloat(String(qs.lng ?? ''))
  const name = String(qs.name ?? '')
  const details = String(qs.details ?? '')

  if (Number.isFinite(lat) && Number.isFinite(lng) && map && markerLayer) {
    // 清理当前覆盖物，但保留大区/缩放监听
    ...
    const m = L.circleMarker([lat, lng], {
      pane:'markerPaneTop', radius: 8, color:'#ef4444', weight:2,
      fillColor:'#ef4444', fillOpacity:0.95
    }).bindPopup(`<b>${escapeHtml(name)}</b><br>${escapeHtml(details)}`)
      .addTo(markerLayer)
